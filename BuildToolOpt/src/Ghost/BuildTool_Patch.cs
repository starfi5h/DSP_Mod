			...
			try
			{
				var codeMacher = new CodeMatcher(instructions)
					...
					.Repeat(matcher => matcher
						.SetAndAdvance(OpCodes.Call, AccessTools.Method(typeof(BuildTool_Patch), nameof(GetItemCountDummy))));
...
#pragma warning restore IDE0060
		...
		public static IEnumerable<CodeInstruction> CheckBuildConditions_Transpiler(IEnumerable<CodeInstruction> instructions)
        {
			try
			{
                ...
                {
					Plugin.Log.LogWarning("Can't find EBuildCondition.NotEnoughItem");
					return instructions;
				}
			...
			try
			{
				...
				switch (__originalMethod.DeclaringType.Name)
                {
					case "BuildTool_Click":
						ToLdloca(list[3]);
						list[4] = new CodeInstruction(OpCodes.Call, AccessTools.Method(typeof(BuildTool_Patch), nameof(AddPrebuildData_Click)));
						break;

					case "BuildTool_Inserter":
						ToLdloca(list[3]);
						list[4] = new CodeInstruction(OpCodes.Call, AccessTools.Method(typeof(BuildTool_Patch), nameof(AddPrebuildData_Inserter)));
						list.Insert(0, list[0]);
						break;

					case "BuildTool_Path":
						ToLdloca(list[3]);
						list[4] = new CodeInstruction(OpCodes.Call, AccessTools.Method(typeof(BuildTool_Patch), nameof(AddPrebuildData_Addon)));
						list.Insert(0, list[0]);
						break;

					case "BuildTool_Addon":
						ToLdloca(list[3]);
						list[4] = new CodeInstruction(OpCodes.Call, AccessTools.Method(typeof(BuildTool_Patch), nameof(AddPrebuildData_Path)));
						list.RemoveAt(2);
						list.Insert(0, list[0]);
						break;
				}
